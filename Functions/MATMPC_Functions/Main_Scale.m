function [Trans_HoT_BP,Trans_NoT_BP,Trans_HoT_P,Trans_NoT_P,Trans_HoT_B,Trans_NoT_B, Trans_SL_BP, Trans_HOV_BP]  = Main_Scale()

% Note: 'clear all' inside a function can sometimes cause variable scope issues in MATLAB, 
% but it is kept here to match your original structure exactly.
...
x_vals = xmin + (xmax-xmin)*rand(1,Nrand);   
y_vals = ymin + (ymax-ymin)*rand(1,Nrand);   
Proxy_Utility_mat = zeros(1,Nrand);          
...
%% ========================================================================
%  RATE & TRANSMITTED DATA CALCULATION
%  ========================================================================
...
Total_SL_BP  = []; Total_HOV_BP = [];

time_draw = 0;
iter_draw = 1;
imax = min([size(state_NoT,1), size(state_HoT,1), size(state_SL,1)]);
...
while time_draw(end) < Tf && iter_draw <= imax
    ...
    % ---------- HOV: PSCA- Position, Beamforming, Phase Shift----------
    R_HOV = eye(3);
    ...
    SL_BP  = Rates_No_Complex_phase(pA,pR,pU_SL,pK,R_SL,theta_SL,f_SL, Power,Bandwidth,freq,beta_B,beta_R);
    HOV_BP = Rates_No_Complex_phase(pA,pR,pU_HOV,pK,R_HOV,theta_HOV,f_HOV, Power,Bandwidth,freq,beta_B,beta_R);
    ...
    Total_SL_BP  = [Total_SL_BP;  sum(SL_BP)];
    Total_HOV_BP = [Total_HOV_BP; sum(HOV_BP)];
...
Trans_SL_BP_list  = cumsum(Total_SL_BP)  * Ts;
Trans_HOV_BP_list = cumsum(Total_HOV_BP) * Ts;
...
% Assign Outputs (Final scalar values at Tf)
Trans_NoT_BP = Trans_NoT_BP_list(end);
Trans_HoT_BP = Trans_HoT_BP_list(end);
Trans_SL_BP  = Trans_SL_BP_list(end);
Trans_HOV_BP = Trans_HOV_BP_list(end);
