...
function Model_Generation_Scale(R_min,K_User,N_A,M_size)
...
settings.K_User = K_User;   % You choose
settings.N_A    = N_A;
settings.M_size = M_size;
settings.R_min = R_min;


settings.model='GTMR_4_com_soft_scale'; % see the folder "examples" for details
...
switch settings.model
    ...
    case 'GTMR_4_com_soft_scale'
        ...
        % Communication variables to be saved from the original
        % GTMR_4_com_soft file and to be reused later in Simulation  and
        % Main files
        
        settings.K_User=K_User;
        settings.pK=pK;
        settings.p_bar_User=p_bar_User; 
        settings.R_min=R_min;
        settings.pA =pA;
        settings.pR=pR;
        settings.freq=freq;
        ...
        settings.beta_B=beta_B;
        settings.beta_R=beta_R;
        settings.M_ele=M_ele;
        settings.N_A=N_A;
